...
public abstract class SectionedRecyclerViewAdapter<VH extends SectionedViewHolder>
  ...
  public void notifySectionChanged(@IntRange(from = 0, to = Integer.MAX_VALUE) int section) {
    ...
    if (sectionItemCount == 0) {
      Log.d(TAG, "There are no map in section " + section + " to notify.");
      return;
    }
    Log.d(
        TAG, "Invalidating " + sectionItemCount + " map starting at index " + sectionHeaderIndex);
  ...
  /**
   * Toggle whether or not section headers are shown when a section has no map. Makes a call to
   * notifyDataSetChanged().
   */
